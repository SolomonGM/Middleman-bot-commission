...
from litecoinrpc import Proxy
LITECOIN_RPC_URL = ""
rpc = Proxy(LITECOIN_RPC_URL)
...
class TransactionMonitor:
        ...
        try:
            ...
            while datetime.now() < end_time:
                ...
                for txn in transactions:
                    txn_hash = txn["hash"]
                    if txn_hash in self.processed_transactions:
                        continue
    ...
    async def handle_transaction_confirmed(self, txn, txn_hash, value_ltc, value_usd):
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description=(
                    f"The transaction has been confirmed with 6/6 confirmations.\n\n"
                    f"**Amount:** ${value_usd:.2f} USD (~{value_ltc:.6f} LTC)\n"
                    f"**Transaction Hash:** `{txn_hash}`\n\n"
    ...
    async def update_confirmation_embed(self, confirmations):
        ...
        try:
            ...
            embed.description = re.sub(r"Confirmations: \*\*\d+/6\*\*", f"Confirmations: **{confirmations}/6**", embed.description)
    ...
    async def log_transaction_to_database(self, txn_hash, value_usd, value_ltc):
        ...
        try:
            ...
            await add_ticket(
                ...
                sender_wallet_address=None,  # Placeholder for sender's wallet address
                receiver_wallet_address=self.crypto_address  # Receiver's wallet address
            )
...
class TradeConfirmationView(discord.ui.View):
    ...
    def is_valid_ltc_address(self, address: str) -> bool:
        """
        Validates if the provided Litecoin address matches the expected format.
        """
        valid_prefixes = ("L", "M", "ltc1")
        min_length = 26  # Minimum possible Litecoin address length
        max_length = 42  # Maximum possible Litecoin address length
    
        return (
            isinstance(address, str) and
            address.startswith(valid_prefixes) and
            min_length <= len(address) <= max_length
        )
    ...
    async def prompt_for_ltc_address(self, channel: discord.TextChannel):
        ...
        address_prompt_embed = discord.Embed(
            ...
            description=(f"{self.receiver.mention}, please provide your Litecoin wallet address where you would like to receive "
        ...
        while True:
            ...
            try:
                address_message = await self.bot.wait_for("message", timeout=120.0, check=address_check)
                ...
                timeout_embed = discord.Embed(
                    title="Timeout",
                    description=f"{self.receiver.mention}, you took too long to provide a valid address. Please try again.",
                    color=discord.Color.red()
                )
                await channel.send(embed=timeout_embed)
                break
    ...
    async def send_ltc(self, to_address: str, amount: float):
        try:
            ...
            # Ensure the sender's wallet has sufficient balance
            balance = rpc.getbalance()
            if balance < amount_ltc:
                raise ValueError("Insufficient funds in the wallet.")

            # Send the transaction
            tx_hash = rpc.sendtoaddress(to_address, amount_ltc)
